...
def insert():
    ...
    if(name=="" or email=="" or phone ==""):
        ...
        con = mysql.connect(host = "localhost",user="root",password="admin",database="address_book")
        cursor = con.cursor()
        cursor.execute("insert into student values('"+ name +"','"+ email +"','"+phone +"')")
...
def delete():
    if(e_name.get() == ""):
        ...
        con = mysql.connect(host = "localhost",user="root",password="admin",database="address_book")
        cursor = con.cursor()
        cursor.execute("delete from student where name ='"+ e_name.get()+"'")
        cursor.execute("commit");
...
def update():
    ...
    if(name=="" or email=="" or phone ==""):
        ...
        con = mysql.connect(host = "localhost",user="root",password="admin",database="address_book")
        cursor = con.cursor()
        cursor.execute("update student set email = '"+email +"',phone = '"+phone +"' where name ='"+name+"'")
...
def get():
    if(e_name.get() == ""):
        ...
        con = mysql.connect(host = "localhost",user="root",password="admin",database="address_book")
        cursor = con.cursor()
        cursor.execute("select * from student where name='"+e_name.get() +"'")
